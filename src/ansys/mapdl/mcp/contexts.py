...
def register_context_tools(mcp):
   ...
   **ALWAYS** start the postprocessing section with a descriptive comment:
   ...
   2. **Vector Plots**
      ...
      - Use MAPDL plotting backend for these plots, and MUST capture plotter:
      ```python
      >>> from ansys.mapdl.core.plotting import GraphicsBackend
      >>> plotter = mapdl.aplot(graphics_backend=GraphicsBackend.MAPDL, return_plotter=True)
      >>> image_data = save_plot(plotter, return_base64=True)
      >>> print(image_data)
      ```
   ...
   ### CRITICAL: How to Display Plots

   **IMPORTANT**: This environment uses non-interactive backends (Agg for Matplotlib, OFF_SCREEN for PyVista).
   You MUST use the provided helper functions to display plots. DO NOT use `plt.show()` or just call plot methods without capturing output.

   **For PyMAPDL/PyVista plots** (e.g., `mapdl.post_processing.plot_*`, `mapdl.aplot()`, etc.):
   ```python
   # Capture the plotter object with return_plotter=True
   plotter = mapdl.post_processing.plot_nodal_solution("USUM", return_plotter=True)
   # Convert to base64 for display
   image_data = save_plot(plotter, return_base64=True)
   print(image_data)  # This displays the image in the MCP client
   ```

   **For Matplotlib plots**:
   ```python
   # Create your matplotlib plot as usual
   plt.figure()
   plt.plot(x_data, y_data)
   plt.xlabel("X Label")
   plt.ylabel("Y Label")
   plt.title("Plot Title")
   # DO NOT use plt.show() - use save_matplotlib_plot instead
   image_data = save_matplotlib_plot(return_base64=True)
   print(image_data)  # This displays the image in the MCP client
   ```
   ...
   3. **Process and visualize** using NumPy and Matplotlib:
      ...
      # Use save_matplotlib_plot to display (NOT plt.show())
      image_data = save_matplotlib_plot(return_base64=True)
      print(image_data)
      ```
   ...
   # Plot nodal solution - MUST capture plotter and use save_plot
   plotter = mapdl.post_processing.plot_nodal_solution("USUM", return_plotter=True)
   image_data = save_plot(plotter, return_base64=True)
   print(image_data)
   ...
   # Plot element solution - MUST capture plotter and use save_plot
   plotter = mapdl.plot_element_solution("SEQV", return_plotter=True)
   image_data = save_plot(plotter, return_base64=True)
   print(image_data)
   ```
   ...
   ## CRITICAL: Plotting in MCP Environment

   **ALWAYS** capture and convert plots to base64 for display. This environment uses non-interactive backends:
   - For PyMAPDL/PyVista plots: Use `return_plotter=True` and `save_plot(plotter, return_base64=True)`
   - For Matplotlib plots: Use `save_matplotlib_plot(return_base64=True)` instead of `plt.show()`
   - Always `print()` the returned base64 data to display the image in the MCP client

   See "CRITICAL: How to Display Plots" in the postprocessing section for detailed examples.
